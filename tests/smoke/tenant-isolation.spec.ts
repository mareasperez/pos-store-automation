...
test('@smoke @regression @manual active tenant matches TEST_TENANT_ID after login', async ({
  page,
}) => {
...
test('@smoke @regression @manual ensureCorrectTenant helper pins the expected tenant', async ({
  page,
}) => {
  test.skip(!config.tenantId, 'Set TEST_TENANT_ID (or E2E_TENANT_ID) to run this check.');
